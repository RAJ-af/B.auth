...
IN="$(cat)"
# argv passthrough: the heredoc below REPLACES python's stdin, so the payload
# must ride in as an argument or every live invocation would exit 65.
python3 - "$MODE" "$IN" <<'PY'
import json, os, sys
mode = sys.argv[1]
try:
...
**Interfaces:**
- Consumes: existing `keycloak.get_discovery/build_authorize_url/exchange_code/LoginStateStore` (unchanged, reused verbatim); `auth.JWTVerifier.verify` via `get_verifier()`; `Settings.allowed_redirect_uris`. CORRECTED at the Wave B gate: (a) Keycloak 26 honors NO port wildcards in valid redirect URIs — the seeded client and Settings both pin the admin callback literally; (b) the callback's redirect_uri must use `Settings.api_public_url` (`http://localhost:${API_PORT}`), NOT `kc_frontend_url` — that names Keycloak's issuer base, so the TOTP redirect would land on Keycloak (405) instead of the API.
- Produces:
